function capPolicyToHtml {
    ...
    LOGIC
    Sets up 2 lookup caches to avoid repeat Graph calls for the same ID.
    Runs 12 loops over the returned collection.
    ...
    Calls      : capLocationToHtml, directoryObjectListResolve, directoryObjectResolve, directoryRoleListResolve, guidVerify, servicePrincipalListResolve
...
$includeApps = directoryObjectListResolve -srcObject $policy.Conditions.Applications.IncludeApplications -getAttrib "displayName"
$excludeApps = directoryObjectListResolve -srcObject $policy.Conditions.Applications.ExcludeApplications -getAttrib "displayName"
...
$includeServicePrincipals = servicePrincipalListResolve -srcObject $policy.Conditions.ClientApplications.IncludeServicePrincipals
$excludeServicePrincipals = servicePrincipalListResolve -srcObject $policy.Conditions.ClientApplications.ExcludeServicePrincipals
...
try {
    $outHtml += capLocationToHtml -type "IncludeLocation" -locations $policy.conditions.Locations.IncludeLocations 
    $outHtml += capLocationToHtml -type "ExcludeLocation" -locations $policy.conditions.Locations.ExcludeLocations 
} catch {
...
$includeUsers = directoryObjectListResolve -srcObject $policy.Conditions.Users.IncludeUsers -getAttrib "userPrincipalName"
$excludeUsers = directoryObjectListResolve -srcObject $policy.Conditions.Users.ExcludeUsers -getAttrib "userPrincipalName"
...
$policy.Conditions.Users.IncludeGroups | ForEach-Object {
    if (guidVerify -InputObject $PSItem) {
        $group = directoryObjectResolve -InputObject $PSItem
        $includeGroups.Add($group) | Out-Null

        $groupMembers = capGroupMemberGet -groupId $group.Id
...
$includeRoles = directoryRoleListResolve -srcObject $policy.Conditions.Users.IncludeRoles
$excludeRoles = directoryRoleListResolve -srcObject $policy.Conditions.Users.ExcludeRoles
